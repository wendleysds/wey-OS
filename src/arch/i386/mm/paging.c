...
pgd_t* pgd_alloc(){
	return kcalloc(PGD_MAX_PTE, sizeof(pgd_t));
}
...
int pgd_unmap(uintptr_t virtaddr){
	...
	kfree_phys(pde[dir_idx] & PAGE_MASK);
	pde[dir_idx] = 0;
...
// duplicate page directory from current loaded directory(because self mapping is used) to dest
...
int pgd_copy(pgd_t* dest, pgd_t* src){
	for (int i = 0; i < PGD_MAX_PTE; i++) {
		dest[i] = src[i];
	}
