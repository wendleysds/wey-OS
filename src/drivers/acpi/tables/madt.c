...
static __init inline void acpi_parse_madt_lapic(struct acpi_madt_lapic *entry) {
...
static __init inline void acpi_parse_madt_ioapic(struct acpi_madt_ioapic *entry) {
	printk("ACPI: Found I/O APIC with GSI base %u at %p\n",
			entry->gsi_base, (void *)entry->ioapic_address);
}

static __init inline void acpi_parse_madt_int_override(struct acpi_madt_int_override *entry) {
	printk("ACPI: Interrupt override for IRQ %u to GSI %u (bus %u)\n",
			entry->source, entry->gsi, entry->bus);
}

static __init inline void acpi_parse_madt_local_nmi(struct acpi_madt_local_nmi *entry) {
	printk("ACPI: Local APIC NMI for processor %u on LINT %u\n",
			entry->processor_id, entry->local_apic_lint);
}

static __init inline void acpi_parse_madt_local_x2apic(struct acpi_madt_local_x2apic *entry) {
